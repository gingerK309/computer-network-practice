...
def check_response_udp(sent_packet, recv_packet):
    ...
    ref_udp_hdr = IPHdr.disassemble(ref_ip_hdr.data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-I', type=int, nargs='?', metavar='ICMP ECHO',help='using ICMP_ECHO_REQUEST',
            ...
            for hop in range(1, max_hop+1):
                ...
                for  request_packet in generate_request(sock=send_socket,count=3, echo_id=echo_id, length=packet_size,
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as recv_socket:
                        ...
                        received_time, addr, finished= recv_resp(sock=recv_socket,echo_id=echo_id,protocol=send_protocol,
                                                                 timeout=timeout,
