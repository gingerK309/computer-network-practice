...
def print_ping_statistics(dst_addr, sent_packets, recv_packets, rtts):
    ...
    print('round-trip min/avg/max/std-dev = {rtt_min:.3f}/{rtt_avg:.3f}/{rtt_max:.3f}/{rtt_std:.3f} ms'.format(
        rtt_min=min(rtts) * 1000, rtt_avg=rtt_avg*1000, rtt_max=max(rtts)*1000, rtt_std=statistics.stdev(rtts)*1000
    ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('len', type=int,nargs='?',metavar='LENGTH',help='playload length',default=10)
    ...
    parser.add_argument('-i',type=int, metavar='INTERVAL',help='send INTERBAL',default=1)
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as sock:
            ...
            for echo_req in gen_echo_req(count=count, data_len=length, id=echo_id):
                ...
                if received_time:
                    ...
                    print('{data_length} bytes from {src_addr}: icmp_seq={seq} ttl={ttl} time={rtt:.3f} ms'.format(
                        data_length=data_length, src_addr = src_addr, seq=seq, ttl=ttl, rtt=rtt*1000))
